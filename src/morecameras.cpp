...
const char *check_output(const char *cmd) {
  ...
  std::string result="";
  while(!feof(pipe)) {
    if(fgets(buf,sizeof(buf),pipe)!=0) { result+=buf; }
  }
  pclose(pipe);
  return(result.c_str());
}
...
static vector<sizeentry> read_sizes(const char *fname) {
  ...
  char fmt[256]; sprintf(fmt,"%%%ds %%d %%d\n",sizeof(se.name));
  ...
  }
  {
    ...
    for(int cid=0;cid<nc;cid++) {
      if(!cam[cid].isValid()) {
        ...
        Eigen::Vector3d c_ww; cam[cid].cam2world(Eigen::Vector3d(0,0,0),c_ww);
    ...
    for(int p=0;p<np;p++) {
      ...
      if((p % 2000)==0) {
        ...
        printf("pos_wc=(%g,%g,%g) cone.direction=(%g,%g,%g) cone.halfangle=%g deg (%g deg)\n",p_wc(0),p_wc(1),p_wc(2),T_w(0),T_w(1),T_w(2),degrees(acos(minV)),degrees(acos(minvlist[0])));
      }
    }
    printf("Calculating viewList ...\n");
    ...
    for(int p=0;p<np;p++) {
      ...
      for(int cid=0;cid<nc;cid++) { is_original[cid]=0; }
      for(int vc=0;vc<nvc;vc++) {
        int cid=pi.viewList[vc].camera;
        if(!cam[cid].isValid()) continue;
        is_original[cid]=1;
      ...
      for(int cid=0;cid<nc;cid++) {
        ...
        if(T_w.dot(t_w)>=minV || is_original[cid]) {
          ...
          if(se.w>0 && se.h>0) {
            Eigen::Vector2d p_i; cam[cid].world2im(p_ww,p_i,true,se.w,se.h);
